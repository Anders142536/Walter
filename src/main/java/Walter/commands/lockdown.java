...
public class lockdown extends Command {
    ...
    public void execute(String usedKeyword, MessageReceivedEvent event) throws CommandExecutionException {
        try {
            ...
            if (newState)
                BlackWebhook.SERVERNEWS.sendMessage("The server is now in lockdown. This means new users will no" +
                        " longer get the guest role, effectively locking them out of this server. If you have questions " +
                        "don't hesitate an admin.");
            else
                BlackWebhook.SERVERNEWS.sendMessage("The server is no longer in lockdown.");
